                    ...
                    <div class="featured__column featured__column--big">
                        <div class="entry" style="background-image:url('{{  $bigFeaturedPosts->poster }}');">
                            ...
                            <div class="entry__content">
                                <span class="entry__category"><a href="/category/{{$bigFeaturedPosts->category->id}}">
                                        {{ $bigFeaturedPosts->category->name }}
                                    </a></span>

                                <h1><a href="{{ route('show_post', ['postId' => $bigFeaturedPosts->id]) }}" title="">
                                        {{ $bigFeaturedPosts->title }}</a></h1>
                    ...
                    <div class="featured__column featured__column--small">
                        @foreach($featuredPosts['normal'] as $featuredPost)
                            <div class="entry" style="background-image:url('{{ $featuredPost->poster }}');">
                                ...
                                <div class="entry__content">
                                    <span class="entry__category"><a href="/category/{{$featuredPost->category->id}}">
                                            {{ $featuredPost->category->name }}
                                        </a></span>

                                    <h1><a href="{{ route('show_post', ['postId' => $featuredPost->id]) }}" title="">
                                            {{ $featuredPost->title }}
